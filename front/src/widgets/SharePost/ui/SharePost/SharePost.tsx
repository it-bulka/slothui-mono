...
export const SharePost = ({ postId, text, showText = true, className }: { postId: string; text?: string; showText?: boolean, className?: string }) => {
  const { isOpen, open, close } = useModalControl()
  return (
    <>
      <SharePostButton onClick={open} showText={showText} className={className}/>
      <ShareDrawer isOpen={isOpen} onClose={close} payload={{ type: 'post', postId }} titleToShare={text} />
    </>
  )
}
