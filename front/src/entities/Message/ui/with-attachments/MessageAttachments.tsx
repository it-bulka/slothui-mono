  ...
  return (
    <div className="attachments">
      ...
      {(attachments.images.length > 0 || attachments.video.length > 0) && (
        <MediaGrid list={[...attachments.images, ...attachments.video ]} />
      )}

      {/* AUDIO */}
      {attachments.audio.length > 0 && (
        <AudioList list={attachments.audio} limit={audioLimit}/>
      )}

      {/* DOCUMENTS */}
      {attachments.file.length > 0 && (
        <DocumentsList list={attachments.file} limit={docLimit}/>
      )}
    </div>
  );
};
